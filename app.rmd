...
best_players <- function(data, position, var){
temp <- data%>%
        filter(Position==position)%>%
        top_n(10, (!!as.symbol(var)))
...
players_name <- function(data, position, var1, var2, var3){

temp <- data%>%
        filter(Position==position)%>%
        top_n(10, (!!as.symbol(var1)))
...
suppressWarnings(renderPlotly({
     best_players(data, input$position, input$y)
   }))
```
...
```{r}
suppressWarnings(renderPlotly({
     players_name(data, input$position, input$y, input$x, input$fill)
   }))
```
