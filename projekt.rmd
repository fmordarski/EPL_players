...
```{r, include=F, echo=F}
defenders <- data%>%filter(Position=='Defender')
...
ggplot(defenders%>%filter(Appearances>=10))+
  ...
  geom_smooth(aes(x=Tackles/Appearances, y=Goals.conceded/Appearances),
              method = "lm", se = FALSE)+
  theme_minimal()+
  labs(title="Liczba wślizgów na mecz a liczba straconych bramek na mecz w podziale na odsetek czystych kont na mecz", x="Liczba wślizgów na mecz", y="Liczba straconych bramek na mecz")
```
